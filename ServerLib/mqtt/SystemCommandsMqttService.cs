...
public class SystemCommandsMqttService(IMqttBaseService mqtt, MqttConfigModel mqtt_conf) : ISystemCommandsService
{
    ...
    public async Task<ResponseBaseModel> CommandDelete(int comm_id, CancellationToken cancellation_token = default)
    {
        ...
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(new SimpleIdNoiseModel() { Id = comm_id }, $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.System}/{GlobalStatic.Routes.Commands}/{GlobalStatic.Routes.DELETE}", cancellation_token);
    ...
    {
        ResponseBaseModel res = new();
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(new SimpleIdNoiseModel() { Id = comm_id }, $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.System}/{GlobalStatic.Routes.Commands}/{GlobalStatic.Routes.START}", cancellation_token);
    ...
    public async Task<TResponseModel<List<SystemCommandModelDB>>> CommandsGetAll(CancellationToken cancellation_token = default)
    {
        ...
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(new NoiseModel(), $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.System}/{GlobalStatic.Routes.Commands}/{GlobalStatic.Routes.LIST}", cancellation_token);
        ...
        TResponseModel<List<SystemCommandModelDB>>? response_mqtt = JsonConvert.DeserializeObject<TResponseModel<List<SystemCommandModelDB>>>(rpc.Response);
    ...
    {
        ResponseBaseModel res = new();
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(comm, $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.System}/{GlobalStatic.Routes.Commands}/{GlobalStatic.Routes.UPDATE}", cancellation_token);
