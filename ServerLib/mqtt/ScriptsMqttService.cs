...
public class ScriptsMqttService(IMqttBaseService mqtt, MqttConfigModel mqtt_conf) : IScriptsService
{
    ...
    {
        TResponseModel<List<ScriptModelDB>> res = new();
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(new LongIdNoiseModel() { Id = script_id }, $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.Scripts}/{GlobalStatic.Routes.DELETE}", cancellation_token);
        ...
        TResponseModel<List<ScriptModelDB>>? response_mqtt = JsonConvert.DeserializeObject<TResponseModel<List<ScriptModelDB>>>(rpc.Response);
    ...
    public async Task<ResponseBaseModel> ScriptEnableSet(ObjectStateModel req, CancellationToken cancellation_token = default)
    {
        ...
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(req, $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.Scripts}/{GlobalStatic.Routes.ENABLE}", cancellation_token);
    ...
    {
        TResponseModel<List<ScriptModelDB>> res = new();
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(new NoiseModel(), $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.Scripts}/{GlobalStatic.Routes.LIST}", cancellation_token);
    ...
    {
        TResponseModel<List<ScriptModelDB>> res = new();
        TResponseModel<string> rpc = await mqtt.MqttRemoteCall(script, $"{mqtt_conf.PrefixMqtt}{GlobalStatic.Routes.Scripts}/{GlobalStatic.Routes.UPDATE}", cancellation_token);
