...
* v3 представляет из себя полноценное стабильное приложение. Для серверной части использовался WebAsm (client side) + Backend через REST/API доступ. С выходом NET8 решил переделать серверную часть на WebApp (InteractiveServerRenderMode)
