...
## Введение
Сервер + клиент (android,win) для доступа к контроллерам умного дома [ab-log](https://ab-log.ru/) (а так же доступ через TelegramBot):
...
- [x] Сценарии: пакеты команд с базовой логикой ветвления по условию
- [x] Триггеры: автозапуск сценариев/команд по событию
...
1. Удалённая настройка сервера возможна только через Telegram. Штатные удалённые клиенты как и серверная часть конфигурируют только своё персональное/локальное MQTT подключение. Для возможности удалённо настраивать серверное приложение без доступа к нему локально - потребуется доступ через Telegram. 
