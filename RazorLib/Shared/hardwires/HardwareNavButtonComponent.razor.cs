...
public partial class HardwareNavButtonComponent : ReloadPageComponentBaseModel
{
    ...
    {
        ResponseBaseModel rest = await Hardwires.SetNamePort(PortEntry);
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (path_parameters.Count == 1 && port_num.IsMatch(path_parameters.Get("pt") ?? ""))
        {
            await InvokeAsync(async () =>
            {
                ...
                TResponseModel<EntryModel> rest = await Hardwires.CheckPortHardware(new PortHardwareCheckRequestModel() { PortNum = uint.Parse(path_parameters.Get("pt")!), HardwareId = Id, CreatePortIfNotExist = true }, CancellationToken);

                Snackbar.ShowMessagesResponse(rest.Messages);
                if (rest.IsSuccess)
                {
                    PortEntry = rest.Response ?? new();
